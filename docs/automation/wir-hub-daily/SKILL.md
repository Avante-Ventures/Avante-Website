...
- **Content-engine (FS local opcional)**: `/Users/cristianjaviermendivelsohincapie/Claude/projects/avante/wir-content-engine/` — items em `items/*.json`; finais em `outputs/{coleção}/{lang}/final/{slug}.json`. (Só rode o sync se essa pasta estiver montada no ambiente.)
...
3. Para cada calendário visível dos dois: `list_events` em [now−1d, now+7d], SÓ eventos de negócio (WIR/seguradoras/prospects: AXA, Mapfre, Generali, NEWE, Sura, HDI, Zurich, BLOPS, Stere, CQCS, MGAs, LP/investidores, governança, marketing, parcerias). EXCLUA pessoal/família/saúde/academia/escola/"Escritório". Upsert em Reuniões por "Event ID": se já existe, atualize Reunião, Início, Participantes, OKR e "Confiança IA"; se não existe, crie com esses campos + Event ID. **Participantes** deve incluir o email do dono do calendário (`nicholas@wirinnovation.ai` ou `jcdepaula@wirinnovation.ai`) e os emails dos convidados, todos prefixados com o dono: "[Nicholas] …" ou "[JC] …". Cap 60/run.
...
Use the **Linear plugin MCP** (not Notion search): `list_issues` with `team` = "cfe004d7-4ad5-415f-9135-c929bf7e0fee" (team Wir), `limit` 60, `includeArchived` false, sem filtrar por status. Mirror the live board without stale opens: for Done/completed or Canceled issues, update an existing Tarefas row to Status="Concluído" and Atualizado=today, but do not create new rows for them. For each active issue, upsert into Tarefas (Linear) keyed on "ID Linear" = the issue **identifier** (e.g. `WIR-63`, taken from the issue url `.../issue/WIR-63/...`) — NOT the uuid; the mirror was seeded 2026-06-11 with 37 active issues keyed this way:
- **Status** map from Linear `status` (the mirror select now has: A fazer, Aberto, Em progresso, Em revisão, Bloqueado, Concluído): "Backlog"/"Todo"/unstarted → "A fazer"; "In Progress"/started → "Em progresso"; "In Review" → "Em revisão"; "Blocked" → "Bloqueado"; "Done"/completed/canceled → "Concluído" (only update existing rows; do not create completed/canceled rows).
...
For active issues: if a row with that ID Linear exists, UPDATE its Status/Responsável/Atualizado; else CREATE. Do not duplicate, do not delete existing rows. This makes the espelho a true daily snapshot of the live board. Cap 60/run.

## Step 3b — Content-engine → Calendário (sync local)
Quando a pasta local do content-engine estiver montada, o objetivo é manter o **Calendário de Conteúdo** espelhando os ~34 artigos HUB do engine, SEM duplicar as linhas semeadas à mão (LinkedIn/Substack/GEO/Vídeo). Se `…/wir-content-engine/` não existir ou não estiver acessível, registre no resumo e pule o Step 3b inteiro sem erro.
1. Liste `…/wir-content-engine/items/*.json` somente após confirmar que a pasta existe. De cada item leia: `slug`, `collection` (→ **Coleção** inteligencia-seguros|guias-automacao), `i18n.pt.title` (→ **Peça**), `languages`.
...
## Step 8 — GSC (optional)
If GSC MCP available, get yesterday's performance for sc-domain:wirinnovation.ai (clicks, impressions, CTR, avg position, top query). Create a Resumo GSC row keyed on yesterday (Dia="{yesterday YYYY-MM-DD}", Data=yesterday, Cliques, Impressões, CTR 0–1, Posição Média, Top Consulta) unless one for yesterday's date exists. Skip silently if unavailable.
...
5. **NUNCA mexa nas vistas-chart** (donut/barras/linha do Dashboard e das DBs): são geridas na UI. Fora a rolagem obrigatória das duas vistas-lista com filtros absolutos no item 3, o robô só insere/atualiza **FILAS**, jamais view-configs — cada chart referencia a propriedade pelo NOME (Status/Pilar/Lane/Idioma/Data), então renomear uma propriedade quebra silenciosamente o groupBy dos charts. Se precisar renomear, faça na UI e reconfigure os charts lá.

## Success criteria
New Briefings page for today + cockpit updated (semáforo, briefing, KPI row) + Reuniões upserted without dupes (Event ID) + Etapa Anterior in sync + Tarefas (Linear) refreshed + **Calendário sincronizado com o engine se a pasta estiver montada (Slug upsert, sem mexer em linhas humanas)** + **Métricas da semana ISO upsertada (só campos do robô)** + **Painel MKT glance refrescado na página Marketing** + Slack row for today + GSC row for yesterday if GSC MCP is available. End with a 3-line PT-BR summary of what was done.
